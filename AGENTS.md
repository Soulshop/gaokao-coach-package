...
- 独立 Web 项目（`~/gaokao-coach-web`）在自己的 `docs/architecture.md` 维护内部架构。本仓库图二只保留 Web 外部入口，不展开 Web 内部节点。各项目维护自己的内部架构，仅在接入边界变化时同步边界描述。
...
- 未发布版本之间的目录快照（如内部 v2→v3）若含同 ID 内容变更，迁移会按冲突拒绝并留备份；操作者确认本地无演化后可用 `--force`：同 ID 节点以模板内容为准覆盖，模板外本地专有节点**保留**——`--force` 不是整文件复制，有本地专有节点时不要当 `cp` 用（无本地演化时两者等价）。对外发布的相邻 tag 之间必须保持追加兼容（只增节点与别名、不改已发布节点内容）。
