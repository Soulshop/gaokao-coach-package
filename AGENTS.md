...
- 未发布版本之间的目录快照（如内部 v2→v3）若含同 ID 内容变更，迁移会按冲突拒绝并留备份；操作者确认本地无演化后可用 `--force` 以模板为准覆盖，即 `cp templates/知识节点.json 消费端` 等价路径。对外发布的相邻 tag 之间必须保持追加兼容（只增节点与别名、不改已发布节点内容）。
