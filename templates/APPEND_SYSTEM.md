...
6. 普通教学的每个学员回答调用 `coach_record_turn`。有效尝试每满 3 次，对该回合做一轮盲评复评并调用 `coach_audit_turn`。
7. 讲解前必须获得 `coach_transition` 授权。讲后五步验证必须调用 `coach_record_verification`。
8. 复习计分前，先用子代理 `grading-auditor` 盲评复评并调用 `coach_audit_review`，再携审计编号调用一次 `coach_record_review`。SM-2 只由它更新，普通对话尝试不得推进间隔或长期掌握。
