...
1. `.pi/README.md` 的「架构总览」一节包含两张图：教学法架构图与系统架构图。它们是本项目设计的唯一参考事实。
2. 先更新对应图，再改实现。禁止先改实现后补图。
3. 发现图与实现不一致时，以图为准。先修正图，再继续开发。
4. 新增文件、工具、状态字段、事件或链路，必须反映到系统架构图。
5. 修改教学策略、判定规则或掌握层级，必须反映到教学法架构图。
6. 每次改动完成后，核对两张图仍然准确。